...
4. **Modo `generated`:** al pasar a `issued` se renderiza el PDF (Puppeteer), se guarda en storage y, en AC3, se escribe `legal_snapshot` desde `instance_legal` **y** se incrusta en el PDF. Transición con **lock por certificado** (ver [10 §4.2.1](./10-diseno-codigo-y-anexos.md)). **Alta:** si no hay plantilla de rol ni default, **rechazar** (no crear `pending`). Ver §3.1.
