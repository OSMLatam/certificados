...
4. **Modo `generated`:** al pasar a `issued` se renderiza el PDF (Puppeteer), **put a MinIO y después** update en Postgres (nunca `issued` sin objeto; [10 §4.2.2](./10-diseno-codigo-y-anexos.md)), y en AC3 se escribe `legal_snapshot` desde `instance_legal` **y** se incrusta en el PDF. Transición con **lock por certificado** (ver [10 §4.2.1](./10-diseno-codigo-y-anexos.md)). **Alta:** si no hay plantilla de rol ni default, **rechazar** (no crear `pending`). Ver §3.1.
5. **Modo `pregenerated`:** el archivo ya está en storage desde el upload; al pasar a `issued` se fija `issued_at` y, en AC3, se escribe el mismo `legal_snapshot` (sin re-render ni tocar el archivo). Un pregenerado sin archivo es error de import, no de emisión.
