...
6. **Storage MinIO** — bucket, acceso; **cambiar** `minioadmin` en prod (boot lo rechaza); **put → luego DB**; GC **solo** huérfanos `certs/` (>24 h sin fila `stored_files`). Los PDF `issued`/`revoked` **no** se borran por ese GC.
