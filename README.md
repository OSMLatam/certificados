...
| AC3 | https://certificados.ac3.org.co *(tentativo DNS)* |
...
| **3** | Badges OSM, jobs Overpass, Turnstile, SMTP | Fase 2 en osm.lat |
...
Incluye tests unitarios e integración (docs/09 §11). Genera apps/api/openapi.yaml según outline docs/10 §13. Implementa GET /health y GET /ready (docs/10 §8). Usa docs/anexos/.env.example y seeds YAML. Al terminar, cumple criterios de aceptación §2.4.
```
...
No implementes: osm_activity, import awardees OSM, jobs Overpass.
...
Añade: osm_profiles, BadgeClass osm_activity, import CSV awardees (osm_id), job BullMQ + Overpass/OSM API, búsqueda por osm_id, vinculación OSM↔email (HU-10.5), Turnstile, SMTP opcional, README operación.

Firma OBv3 queda NULL. Mock de APIs OSM en CI; test live opcional.
